    ...
    vowels: string[] = ['a', 'e', 'i', 'o', 'u', 'y'];
  	...
  	randomize(pIndex: number, numOfWords: number, noRepeat: boolean) {
        ...
        while (i < this.numOfWords && failedAttempts < 1000) {
        	let timeoutHandle = setTimeout(() => {return false;}, 5000);
        	...
        	let j = 0;
	        while (j < (this.patterns[this.pIndex].length - this.startingPoint.length)) {
	        	let letter = '';
	        	if (this.patterns[this.pIndex].charAt(j + this.startingPoint.length) == '0') {
        ...
        if (failedAttempts == 1000) {
            this.error = 'Whops! Ran out of words :( (' + this.words.length + ' generated)';
        }
